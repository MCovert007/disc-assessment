...
const UserSetting: React.FC = () => {
  ...
  const updateUser = async () =>{
    ...
    // const oldpassword = form.current.oldpassword.value
    const password = form.current.password.value
    const repassword = form.current.repassword.value
    ...
    const passwordSchema = z.string().min(8); // minimum length of 6 characters
    try {
      emailSchema.parse(email);
      passwordSchema.parse(password);
    } catch (error:any) {
      const messageJson = JSON.parse(error.message)
      if(messageJson[0]["message"])
        alert(messageJson[0]["message"])
      return
    }
    ...
    try {
      ...
      const response = await axios.put(`/api/db/users`, data)
      if(response.data.error){        
        alert(response.data.error)
      }else{
        alert("Successful Update")
        await signIn('credentials', { callbackUrl:  "/", redirect: true, email:email, password:password });
      }
    } catch (error:any) {
  ...
  return (
    <div className="sign-page">
      <div className="flex justify-center items-center h-screen backdrop-blur-[4px]">
        <div className="max-w-[440px] md:min-w-[440px] mx-auto rounded-sm bg-white shadow-defaul dark:bg-boxdark bg-opacity-90">
              ...
              <form ref={form}>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input type="text" placeholder="Enter your full name" name="name" defaultValue={session?.user?.name?session?.user?.name:""}
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input type="email" placeholder="Enter your email" name="email" defaultValue={session?.user?.email?session?.user?.email:""} disabled
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 text-black outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                    />
                ...
                <div className="flex justify-center">
                  <Link href="/" className="hover:text-primary">Go Home</Link>
                </div>
              </form>
            </div>
          </div>
